...
class Checkers:
    ...
    def predict_batch(self, states):
        """
        批量用神经网络推理多个状态，返回概率和Q值列表。
        """
        nn_inp = np.stack([np.moveaxis(s[:14], 0, -1) for s in states])
        prob_planes, q_values = self.neural_net.predict(nn_inp)
        prob_planes = prob_planes.reshape((-1, 8, 8, 8))
        q_values = q_values.reshape((-1,))
        return prob_planes, q_values
