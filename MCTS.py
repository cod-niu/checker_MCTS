...
class MCTS:
    ...
    def begin_tree_search(cls, root_node):
        ...
        while cls.computational_budget():
            expand_nodes = []
            cls.collect_expand_nodes(root_node, expand_nodes)
            if expand_nodes:
                # 批量推理
                states = [node.state for node in expand_nodes]
                prob_planes_batch, q_values_batch = cls.game_env.predict_batch(states)
                for node, prob_planes, q_value in zip(expand_nodes, prob_planes_batch, q_values_batch):
                    # 展开所有子节点
                    for _ in range(len(node.unvisited_child_states)):
                        next_state = node.unvisited_child_states.pop()
                        child_node = MCTS_Node(next_state, parent=node)
                        node.children.append(child_node)
                    cls.game_env.set_prior_probs(node.children, prob_planes)
                    node.backpropagation(q_value, node.player)
            else:
                # 没有需要批量推理的节点，执行原有单步逻辑
                root_node.selection()
        if cls.verbose: 
    ...
    @classmethod
    def collect_expand_nodes(cls, node, expand_nodes):
        """
        递归收集所有需要神经网络推理的节点（即unvisited_child_states不为空且用NN的节点）。
        """
        if hasattr(cls, 'neural_net') and cls.neural_net and node.unvisited_child_states:
            expand_nodes.append(node)
        else:
            for child in node.children:
                cls.collect_expand_nodes(child, expand_nodes)
